...
import { ADMIN_DELETED_MISSIONS_QUERY } from "common/utils/apiQueries";
...
export const useRefreshDeletedMissions = () => {
  ...
  const refresh = async () => {
    ...
    if (!deletedMissionsCaches[cacheKey] || now() - deletedMissionsCaches[cacheKey].timestamp >= MINUTE * CACHE_BY_COMPANY_MIN) {
